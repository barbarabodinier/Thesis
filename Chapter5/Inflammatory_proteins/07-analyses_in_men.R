...
# Keeping only Men
covars <- covars[which(as.character(covars$gender) == "Male"), ]
proteins <- proteins[rownames(covars), ]
print(nrow(proteins))
print(all(rownames(proteins) == rownames(covars)))
...
# ensemble=Ensemble(stability=stab, xdata=xdata, ydata = as.numeric(covars$case))
# predictions=EnsemblePredictions(ensemble = ensemble, xdata = xdata)
# myroc=ROC(predicted=predictions, observed=covars$case)

# Saving the results
saveRDS(stab, "Results/Inflammatory_proteins/Multivariate_lung_cancer_men.rds")
saveRDS(expl, "Results/Inflammatory_proteins/Incremental_performances_lung_cancer_men.rds")
