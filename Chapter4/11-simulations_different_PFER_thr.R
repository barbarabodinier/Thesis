...
mycolours <- lighten(c(pal_mb, pal_ss, "darkred", "firebrick", "red", "tomato"), amount = 0.2)
...
for (PFER_thr in PFER_thr_list) {
  for (metric in c("F1_score", "precision", "recall")) {
    ...
    for (simul_id in 1:3) {
      performances <- readRDS(paste0("Results/Stability_selection/2-simulations/1-graphical_model/Simulations_", simul_study_id, "_", topology, "/Performances_", simul_id, "_merged_PFER_thr_", PFER_thr, ".rds"))
      mylist <- list()
      for (k in 5:nrow(performances)) {
      ...
      axis(side = 1, at = xseq[(1:length(pi_list))], labels = pi_list, las = 2)
      axis(side = 1, at = xseq[(length(pi_list) + 1):(length(pi_list) * 2)], labels = pi_list, las = 2)
      ...
      abline(v = c(12, 20), lty = 3, col = "black")
    }
    dev.off()
  }
}
