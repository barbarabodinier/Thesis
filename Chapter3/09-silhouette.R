...
plot(true_sil, type="h", lwd=5, lend=1, bty="n",
...
for (k in 1:nrow(simul$data)){
  axis(side=1, at=k, labels = k, las=2, col.axis=mycolours[simul$theta[k]], 
       tick=FALSE, line=-0.5)
}
for (k in 1:length(unique(simul$theta))){
  axis(side=1, at=range(which(simul$theta==k))+c(-0.25,0.25), labels = NA, line=2)
  axis(side=1, at=mean(which(simul$theta==k)), labels = paste0("Simulated cluster ", k), 
...
text(simul$data, labels = 1:nrow(simul$data))
legend("topright", col=mycolours_estimated, pch=15, 
...
for (k in 1:nrow(simul$data)){
  axis(side=1, at=k, labels = k, las=2, col.axis=mycolours_estimated[pam$clustering[k]], 
       tick=FALSE, line=-0.5)
}
for (k in 1:length(unique(simul$theta))){
  axis(side=1, at=range(which(simul$theta==k))+c(-0.25,0.25), labels = NA, line=2)
