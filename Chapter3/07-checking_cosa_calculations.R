...
theta_xc=rbind(c(rep(1, 5), rep(0, 5)),
               c(rep(1, 4), rep(0, 6)),
               c(rep(1, 4), rep(0, 6)))
set.seed(1)
simul=SimulateClustering(n=c(20,20,20), 
                         theta_xc = theta_xc,
                         ev_xc = 0.9)
simul$theta_xc
...
plot(as.matrix(dist(x))[upper.tri(as.matrix(dist(x)))],
...
plot(as.matrix(rslt_dflt_cosa2$D)[upper.tri(as.matrix(rslt_dflt_cosa2$D))],
     mydist[upper.tri(mydist)])
